...
class LLMThread(models.Model):
    ...
    def _compute_name(self):
        ...
        for thread in self:
            try:
                ...
                record_name = record.display_name if record else f"{thread.res_model},{thread.res_id}"
    ...
    def get_thread_messages(self, trigger_message=None):
        ...
        domain = [
            ...
            ('res_id', '=', self.res_id),
            ('subtype_id', 'in', [self.env.ref('llm_thread.mt_llm_question').id, self.env.ref('llm_thread.mt_llm_answer').id]),
        ]
        if trigger_message:
