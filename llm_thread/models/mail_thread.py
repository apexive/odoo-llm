...
class MailThread(models.AbstractModel):
    ...
    llm_thread_ids = fields.One2many("llm.thread", compute="_compute_llm_thread_ids")
    ...
    def _message_post_after_hook(self, message, msg_vals):
        ...
        if msg_vals.get("subtype_id") == self.env.ref(
            "llm_thread.mt_llm_question"
        ).id and not self.env.context.get(
            "skip_ai_response"
        ):  # Not already an AI response
            llm_thread = self.env["llm.thread"].get_user_thread(self._name, self.id)
            ...
            if llm_thread:
                try:
                    ...
                    if accumulated_content:
                        self.with_context(
                            skip_ai_response=True, mail_create_nosubscribe=True
