...
registerPatch({
  ...
  recordMethods: {
    ...
    postUserMessageForLLM() {
      ...
      this._handleResponsesFromLLM(thread, `/llm/thread/generate?thread_id=${thread.id}&message=${encodeURIComponent(messageBody)}`);
    },
    ...
    repostExistingUserMessagesForLLM() {
      ...
      this._handleResponsesFromLLM(thread, `/llm/thread/generate?thread_id=${thread.id}`);
    },
