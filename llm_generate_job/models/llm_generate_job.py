...
class LLMGenerateJob(models.Model):
    ...
    def process_webhook_result(self, request, webhook_data):
        ...
        if self.provider_id.is_webhook(request):
            if not self.provider_id.verify_webhook(request, webhook_data):
                _logger.error(f"Webhook verification failed for job {self.id}")
